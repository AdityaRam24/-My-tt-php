...
// Check connection
    ...
    if (isset($_POST['Enroll'])) {
        ...
        if ($info[0] != null) { // Check for null value before proceeding
            ...
            $checkcount = "SELECT count(*) as count1 FROM student_selects_slots WHERE subject_name = ? and t_name = ? and S_id = ? and subject_name like '%Lab'";
            $stmtCheck = $conn->prepare($checkcount);
            $stmtCheck->bind_param("sss", $info[2] , $info[1] , $info[0]);
            $stmtCheck->execute();
            $resultCheck = $stmtCheck->get_result();
            $row = $resultCheck->fetch_assoc();
            $numcount2 = $row['count1'];

            $checkcount = "SELECT count(*) as count2 FROM student_selects_slots WHERE subject_name = ? and t_name = ? and S_id = ? " ;
            $stmtCheck = $conn->prepare($checkcount);
            $stmtCheck->bind_param("sss", $info[2] , $info[1] , $info[0]);
            $stmtCheck->execute();
            $resultCheck = $stmtCheck->get_result();
            $row = $resultCheck->fetch_assoc();
            $numcount2 = $row['count2'];
            echo $numcount;
            // S_id exists, proceed with enrollment
            if($numcount+1 < 75){
